...
Function::Function(FunctionAttribute attribute
...
std::vector<DzBaseArgument *> Function::arguments() const
{
	return m_arguments;
}

FunctionAttribute Function::attribute() const
{
	return m_attribute;
}

bool Function::hasMatchingSignature(const EntryPoint &entryPoint, const Stack &values) const
{
...
int8_t Function::signatureCompatibility(const EntryPoint &entryPoint, const Stack &values) const
{
	if (m_arguments.size() != values.size())
	{
		return -1;
...
std::vector<DzResult> Function::build(const EntryPoint &entryPoint, Stack values) const
{
	auto pep = entryPoint
		.withValues(values);
...
std::vector<Function::Argument> Function::handleArgument(DzBaseArgument *argument, const EntryPoint &entryPoint, const BaseValue *value) const
{
