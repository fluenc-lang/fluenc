	...
	)
{
	...
	{
		strategyFor<scalar_value, reference_value>([](auto entryPoint, auto source, auto target, auto& emitter)
		{
			UNUSED(emitter);
		...
		strategyFor<reference_value, reference_value>([](auto entryPoint, auto source, auto target, auto& emitter)
		{
			UNUSED(emitter);
		...
		strategyFor<lazy_value, lazy_value>([](auto entryPoint, auto source, auto target, auto& emitter)
		{
			return target->assignFrom(entryPoint, source, emitter);
		}),
		strategyFor<BaseValue, lazy_value>([](auto entryPoint, auto source, auto target, auto& emitter)
		{
			return target->assignFrom(entryPoint, source, emitter);
		}),
            strategyFor<user_type_value, user_type_value>([](auto entryPoint, auto source, auto target, auto& emitter)
		{
            ...
            strategyFor<tuple_value, tuple_value>([](auto entryPoint, auto source, auto target, auto& emitter)
		{
		...
		strategyFor<BufferValue, BufferValue>([](auto entryPoint, auto source, auto target, auto& emitter)
		{
			...
			auto referenceLoad = builder.createLoad(source->address());

			builder.createStore(referenceLoad, target->address());
...
const scalar_value *ValueHelper::getScalar(const std::shared_ptr<peg::Ast> &ast, const EntryPoint &entryPoint, const BaseValue *value)
{
	if (auto scalar = value_cast<const scalar_value *>(value))
	{
		return scalar;
	}

	if (auto reference = value_cast<const reference_value *>(value))
	{
		IRBuilderEx builder(entryPoint);
	...
	auto &expectedMetadata = scalar_value::staticMetadata();
...
const scalar_value *ValueHelper::getScalar(const std::shared_ptr<peg::Ast> &ast, const EntryPoint &entryPoint, Stack &values)
{
