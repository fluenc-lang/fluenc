...
expanded_type::expanded_type(const std::vector<const Type *> types)
	: m_types(types)
{
}
...
std::string expanded_type::fullName() const
{
	std::stringstream ss;
...
std::vector<const Type *> expanded_type::types() const
{
	return m_types;
}

llvm::Type *expanded_type::storageType(llvm::LLVMContext &context) const
{
...
expanded_type *expanded_type::get(const std::vector<const Type *> &types)
{
...
const Type *expanded_type::iteratorType() const
{
	return m_types[0];
}
