...
std::vector<DzResult> StackSegmentNode::build(const EntryPoint &entryPoint, Stack values) const
{
	...
	std::transform(begin(m_values), end(m_values), index_iterator(), std::back_inserter(orderedValues), [](auto value, auto index) -> Indexed<Node *>
	{
		return { index, value };
	});
